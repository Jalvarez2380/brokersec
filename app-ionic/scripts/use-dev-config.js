...
const CAP_CONFIG_JSON = join(__dirname, '../android/app/src/main/assets/capacitor.config.json');
const DEV_SERVER_URL = process.env.CAP_SERVER_URL || 'http://10.0.2.2:3000';

if (!existsSync(CAP_CONFIG_JSON)) {
  console.error('capacitor.config.json no encontrado. Ejecuta "npx cap sync android" primero.');
  process.exit(1);
}

const config = JSON.parse(readFileSync(CAP_CONFIG_JSON, 'utf8'));
...
writeFileSync(CAP_CONFIG_JSON, JSON.stringify(config, null, 2));
